...
def readFile(filename):
    ...
    with open(filename, 'r') as dataFile:
        ...
        for line in dataFile:
            line = line.strip("\n")
            lineList = line.split(",")
            dataSet1.append(float(lineList[0]))
            dataSet2.append(float(lineList[1]))
            dataset3.append(float(lineList[2]))
...
def calibrateAccelerometer(acc_x, acc_y, acc_z):
    acc_x_error=(acc_x/16384.)/float(len(acc_x))
    acc_y_error=(acc_y/16384.)/float(len(acc_y))
    acc_z_error=(acc_z/16384.)/float(len(acc_z))
